...
// We will start with the helper functions that we will use in our work in the main !


// We here want to convert String to lower case and removing preceeding and trailing spaces
...
// Here we want to read CSV files into a string array
vector<string> read_csv(string filepath)
{
	...
	{
		cout << "Error to open the File" << filepath;
	}
...
// We here want to calculate initial PR
...
int main()
{
	...
	// READING CSV FILES
	vector<string> impressions_file = read_csv("impressions.csv");
	vector<string> keywords_file = read_csv("keyword.csv");
	vector<string> web_graph_file = read_csv("web_graph.csv");

	vector<string> clicks_file = read_csv("clicks.csv");
...
start:
	...
	search:
		...
		else
		{
			...
			update_impressions(sorted_results,impressions_file, "impressions.csv");
		...
		if (answer == '1')
		{
			...
			update_clicks(site, clicks_file, "clicks.csv");
